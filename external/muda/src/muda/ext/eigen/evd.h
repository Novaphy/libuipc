...
namespace eigen
{
     ...
     */
    template <typename T, int N>
    MUDA_GENERIC void evd_jacobi(Eigen::Matrix<T, N, N>& symmetrix,
                                 Eigen::Vector<T, N>&    eigen_values,
                                 Eigen::Matrix<T, N, N>& eigen_vectors)
    {
        // Non-const ref: CoreX clang-CUDA can emit invalid addrspacecast when a
        // fixed-size Eigen::Matrix is passed by value or by const& on __device__
        // (PlainObjectBase / storage-class mismatch for float).
        ...
        // CoreX FPU has ~23-bit mantissa for doubles; use a relative
        // tolerance so that convergence is always reachable.
        T diag_norm = T(0);
        for(int i = 0; i < N; ++i)
            diag_norm += abs(symmetrix(i, i));
        tol = diag_norm * static_cast<T>(1e-4);
        if(tol < static_cast<T>(1e-6))
            tol = static_cast<T>(1e-6);
#endif

        for(int iter = 0; iter < max_iter; ++iter)
        {
            if(details::calc_sumDiagOff(symmetrix) <= tol)
                break;
            int q = -1, p = -1;
            T   max_value;
