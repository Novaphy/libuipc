...
int main()
{
    ...
    // Ensure dynamic backend libraries can be located for Engine's dylib loading.
    // Note: Engine loads backend dylibs during construction, so init() must happen before Engine is created.
    {
        auto uipc_config = uipc::default_config();
        uipc_config["module_dir"] = fs::current_path().string();
