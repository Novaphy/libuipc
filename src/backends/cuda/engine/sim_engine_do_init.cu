...
void SimEngine::build()
{
    ...
    // 1) build all systems
    build_systems();
    logger::info("[cuda] SimEngine::build: resolve engine-aware systems ...");
    ...
    // 3) dump system info
    logger::info("[cuda] SimEngine::build: dump_system_info ...");
    dump_system_info();
    logger::info("[cuda] SimEngine::build: done");
}
