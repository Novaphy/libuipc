...
__global__ void kernel_sym_spmv_grouped_row_atomic(int          n_triplets,
                                                   const int*   rows,
                                                   const int*   cols,
                                                   const Float* blocks,
                                                   const Float* x,
                                                   ...
                                                   Float*       y)
{
    ...
    // Keep the symmetric off-diagonal contribution triplet-parallel. This is
    // independent of row ordering and preserves correctness for all inputs.
    if(bi != bj)
    {
        const Float* B   = blocks + t * 9;
        Float xi0 = x[bi * 3 + 0];
    ...
    Float yi1 = 0;
    Float yi2 = 0;
    for(int k = t; k < n_triplets && rows[k] == bi; ++k)
    {
        int cj = cols[k];
        const Float* B = blocks + k * 9;
        Float xj0 = x[cj * 3 + 0];
        Float xj1 = x[cj * 3 + 1];
        Float xj2 = x[cj * 3 + 2];
