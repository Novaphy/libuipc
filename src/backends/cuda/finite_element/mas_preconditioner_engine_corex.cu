                                                          ...
                                                          SizeT                        newton_iter)
{
    ...
    (void)frame;
    (void)newton_iter;
    // Optional: export cluster_hessians to Matrix Market + JSON metadata.
}
...
void MASPreconditionerEngine::set_preconditioner(
    const Matrix3x3* d_triplet_values,
    const int*             d_row_ids,
    const int*             d_col_ids,
                        ...
                        if(vert_col / BANKSIZE == vert_row / BANKSIZE)
                        {
                            ...
                            if(vert_col >= vert_row)
                            {
                                ...
                                for(int i = 0; i < 3; i++)
                                    for(int j = 0; j < 3; j++)
                                    {
                                        muda::atomic_add(&(cluster_hess[cluster_id].M[si](i, j)), H(i, j));
                                        if(vert_col == vert_row)
                                            muda::atomic_add(&(cluster_hess[cluster_id].M[si](i, j)), H(j, i));
                                    }
                            }
                            else
                            {
                ...
                if(prefix == 1)
                {
                    ...
                    if(is_boundary)
                    {
                        ...
                        while(level < level_num - 1)
                        {
                            ...
                            for(int i = 0; i < 3; i++)
                                for(int j = 0; j < 3; j++)
                                    muda::atomic_add(&(cluster_hess[cid].M[si](i, j)), mat3(i, j));
    ...
    Launch(num_blocks, block_size)
        .apply(
             ...
             total_threads] __device__() mutable
            {
                ...
                using InvWork = Float;
#else
                using InvWork = double;
#endif
                // Load the full symmetric matrix into shared memory (double when Float=double
                // for inversion stability; Float when building in single precision).
                __shared__ InvWork s_mat[32 / BANKSIZE][MAT_DIM][MAT_DIM];
                __shared__ InvWork s_col[32 / BANKSIZE][MAT_DIM];

                const InvWork z = InvWork(0);
                const InvWork one = InvWork(1);
                ...
                for(int row = 0; row < MAT_DIM; row++)
                {
                    ...
                    if(node_col >= node_row)
                    {
                        ...
                        s_mat[block_mat_id][row][col] =
                            static_cast<InvWork>(cluster_hess[mat_id].M[si](row % 3, col % 3));
                    }
                    else
                    {
                        int si = sym_index(node_col, node_row);
                        s_mat[block_mat_id][row][col] =
                            static_cast<InvWork>(cluster_hess[mat_id].M[si](col % 3, row % 3));
                    }
                    // Regularize: zero diagonal → identity (for padding nodes)
                    if(row == col && s_mat[block_mat_id][row][col] == z)
                        s_mat[block_mat_id][row][col] = one;
                }
                ...
                {
                    __syncthreads();
                    InvWork pivot_val = s_mat[block_mat_id][pivot][pivot];
                    ...
                    for(int row = 0; row < MAT_DIM; row++)
                    {
                        if(row != pivot)
                        {
                            InvWork factor = -s_col[block_mat_id][row];
                            __syncthreads();
                            s_mat[block_mat_id][row][col] +=
                ...
                // Mixed precision: store inverse as float; single-precision build: store as Float.
                for(int row = 0; row < MAT_DIM; row++)
                {
                    int node_row = row / 3;
                    int node_col = col / 3;
...
// ---------------------------------------------------------------------------
void MASPreconditionerEngine::build_multi_level_R(const MasVec3* R,
        ...
        .apply(
             ...
             level_num = m_level_num, N] __device__() mutable
            {
                ...
                else
                {
                    ...
                    muda::atomic_add(sum_residual + local_warp_id * BANKSIZE + elected_lane, r[0]);
                    muda::atomic_add(sum_residual + local_warp_id * BANKSIZE + elected_lane + DEFAULT_BLOCKSIZE, r[1]);
                    muda::atomic_add(sum_residual + local_warp_id * BANKSIZE + elected_lane + 2 * DEFAULT_BLOCKSIZE, r[2]);
                    ...
                    if(elected_prefix == 0)
                    {
                        ...
                        {
                            cur = going_next[cur];
                            muda::atomic_add(&(multi_lr[cur][0]), sum_residual[threadIdx.x]);
                            muda::atomic_add(&(multi_lr[cur][1]), sum_residual[threadIdx.x + DEFAULT_BLOCKSIZE]);
                            muda::atomic_add(&(multi_lr[cur][2]), sum_residual[threadIdx.x + DEFAULT_BLOCKSIZE * 2]);
                        }
                    }
                }
            });
    ...
    Launch(num_blocks, block_size)
        .apply(
             ...
             N] __device__() mutable
            {
                ...
                __shared__ float s_R[BANKSIZE][3];
                if(threadIdx.x < BANKSIZE)
                {
                    auto r            = multi_lr[vert_col];
                    s_R[threadIdx.x][0] = r[0];
                    s_R[threadIdx.x][1] = r[1];
                    s_R[threadIdx.x][2] = r[2];
                }
                __syncthreads();

                // Multiply: result = inverse_block * R_col
                Eigen::Vector3f result;
                Eigen::Vector3f rhs{s_R[local_col][0], s_R[local_col][1], s_R[local_col][2]};
                if(vert_col >= vert_row)
                {
                    int si = sym_index(local_row, local_col);
                    result = cluster_inv[cluster_id].M[si] * rhs;
                }
                else
                {
                    int si = sym_index(local_col, local_row);
                    result = cluster_inv[cluster_id].M[si].transpose() * rhs;
                }
...
// ---------------------------------------------------------------------------
void MASPreconditionerEngine::collect_final_Z(MasVec3* Z,
                                              muda::CVarView<IndexT> converged)
{
    using namespace muda;
    int N = m_total_nodes;
    ...
    ParallelFor()
        ...
        .apply(N,
                ...
                level_num] __device__(int idx) mutable
               {
                   ...
                   Z[idx].x = static_cast<Float>(cz.x);
                   Z[idx].y = static_cast<Float>(cz.y);
                   Z[idx].z = static_cast<Float>(cz.z);
               });
}
                                    ...
                                    muda::CVarView<IndexT>        converged)
{
    ...
    build_multi_level_R(reinterpret_cast<const MasVec3*>(r.data()), converged);
    ...
    collect_final_Z(reinterpret_cast<MasVec3*>(z.data()), converged);
}
