#!/usr/bin/env bash
#
# verify_nvidia_branch_equiv.sh (v11 expanded)
# --------------------------------------------
# For every tracked file, extract the NVIDIA branch (UIPC_COREX_CUDA10_COMPAT
# undefined) view and diff it against the same path in libuipc-origin.
# Pass = NVIDIA path is byte-equivalent to upstream origin.
#
# Override REF_DIR env to point to the origin checkout root containing src/.
# Default assumes co-checkout of libuipc-origin alongside this fork.

set -euo pipefail

ROOT_DIR="$(cd "$(dirname "$0")/../.." && pwd)"
REF_DIR="${REF_DIR:-${ROOT_DIR}/../libuipc-origin/libuipc}"
...
for rel in "${FILES[@]}"; do
    ...
    if diff -q "${ref_path}" "${extracted}" > /dev/null; then
        ...
        # Trailing-newline-only diff is cosmetic (C preprocessor no-op).
        a="$(mktemp)"; b="$(mktemp)"
