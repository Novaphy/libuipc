...
namespace
{
constexpr Float revolute_joint_axis_norm_eps()
{
...
constexpr Float revolute_joint_axis_sq_eps()
{
    const Float e = revolute_joint_axis_norm_eps();
    return e * e;
}
}  // namespace
        ...
        info.for_each(
            ...
            [&](const InterAffineBodyConstitutionManager::ForEachInfo& I, geometry::Geometry& geo)
            {
                ...
                for(auto&& [i, e] : enumerate(Es))
                {
                    ...
                    UIPC_ASSERT(Dir.norm() > revolute_joint_axis_norm_eps(),
        ...
        info.for_each(
            ...
            [&](const InterAffineBodyConstitutionManager::ForEachInfo& I, geometry::Geometry& geo)
            {
                ...
                for(auto&& [i, e] : enumerate(Es))
                {
                    ...
                    UIPC_ASSERT(Dir.squaredNorm() > revolute_joint_axis_sq_eps(),
